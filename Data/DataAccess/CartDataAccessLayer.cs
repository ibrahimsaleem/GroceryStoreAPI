...
namespace Data.DataAccess
{
    public class CartDataAccessLayer : ICartService
    {
        ...
        public CartDataAccessLayer(GroceryDBContext dbContext)
        {
            _dbContext = dbContext;
        }

        public void AddGroceryToCart(int userId, int groceryId)
        {
            string cartId = GetCartId(userId);
            int quantity = 1;

            CartItems existingCartItem = _dbContext.CartItems.FirstOrDefault(x => x.ProductId == groceryId && x.CartId == cartId);
        ...
        public void RemoveCartItem(int userId, int groceryId)
        {
            try
            {
                string cartId = GetCartId(userId);
                CartItems cartItem = _dbContext.CartItems.FirstOrDefault(x => x.ProductId == groceryId && x.CartId == cartId);
        ...
        public void DeleteOneCartItem(int userId, int groceryId)
        {
            try
            {
                string cartId = GetCartId(userId);
